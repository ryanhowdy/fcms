<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Hash;
use App\Models\User;
use App\Models\UserSetting;
use App\Models\Address;
use App\Models\NavigationLink;

class InstallController extends Controller
{
    private $totalSteps = 4;

    /**
     * Show the application dashboard.
     *
     * @return Illuminate\Support\Facades\View
     */
    public function index()
    {
        if ($this->isStepThreeComplete())
        {
            return redirect()->route('login');
        }
        else if ($this->isStepTwoComplete())
        {
            return redirect()->route('install.admin');
        }
        else if ($this->isStepOneComplete())
        {
            return redirect()->route('install.config');
        }

        return redirect()->route('install.database');
    }

    /**
     * Has step one (create tables) been completed
     *
     * @return boolean
     */
    private function isStepOneComplete()
    {
        try
        {
            if (Schema::hasTable('addresses'))
            {
                return true;
            }
        }
        catch (\Exception $e)
        {
            return false;
        }
    }

    /**
     * Has step two (configuration) been completed
     *
     * @return boolean
     */
    private function isStepTwoComplete()
    {
        $links = NavigationLink::where('route_name', 'home')
            ->get();

        if ($links->isEmpty())
        {
            return false;
        }

        return true;
    }

    /**
     * Has step three (admin user) been completed
     *
     * @return boolean
     */
    private function isStepThreeComplete()
    {
        $admin = User::select()
            ->where('name', '!=', 'system')
            ->get();

        if ($admin->isEmpty())
        {
            return false;
        }

        return true;
    }

    /**
     * Show the database setup view
     *
     * @return Illuminate\View\View
     */
    public function database()
    {
        $percent = round((1 / $this->totalSteps) * 100);

        return view('install.database', [ 'progress' => $percent ]);
    }

    /**
     * Show the configuration form
     *
     * @return Illuminate\View\View
     */
    public function configurationCreate()
    {
        $percent = round((2 / $this->totalSteps) * 100);

        return view('install.config', [ 'progress' => $percent ]);
    }

    /**
     * Store the configuration in the db
     *
     * @param  Illuminate\Http\Request $request
     * @return Illuminate\Support\Facades\View
     */
    public function configurationStore(Request $request)
    {
        $insertParams = [
            [
                'link'       => gettext('Home'),
                'route_name' => null,
                'group'      => 1,
                'order'      => 1,
            ],
            [
                'link'       => gettext('Home'),
                'route_name' => 'home',
                'group'      => 1,
                'order'      => 2,
            ],
            [
                'link'       => gettext('Calendar'),
                'route_name' => 'calendar',
                'group'      => 1,
                'order'      => 3,
            ],
            [
                'link'       => gettext('Members'),
                'route_name' => 'members',
                'group'      => 1,
                'order'      => 4,
            ],
            [
                'link'       => gettext('Address Book'),
                'route_name' => 'addressbook',
                'group'      => 1,
                'order'      => 5,
            ],
            [
                'link'       => gettext('Communicate'),
                'route_name' => null,
                'group'      => 2,
                'order'      => 1,
            ],
            [
                'link'       => gettext('Discussions'),
                'route_name' => 'discussions',
                'group'      => 2,
                'order'      => 2,
            ],
            [
                'link'       => gettext('Share'),
                'route_name' => null,
                'group'      => 3,
                'order'      => 1,
            ],
            [
                'link'       => gettext('Photos'),
                'route_name' => 'photos',
                'group'      => 3,
                'order'      => 2,
            ],
            [
                'link'       => gettext('Videos'),
                'route_name' => 'videos',
                'group'      => 3,
                'order'      => 3,
            ],
            [
                'link'       => gettext('Misc.'),
                'route_name' => null,
                'group'      => 4,
                'order'      => 1,
            ],
            [
                'link'       => gettext('Contact'),
                'route_name' => 'contact',
                'group'      => 4,
                'order'      => 2,
            ],
            [
                'link'       => gettext('Help'),
                'route_name' => 'help',
                'group'      => 4,
                'order'      => 3,
            ],
            [
                'link'       => gettext('Admin'),
                'route_name' => null,
                'group'      => 5,
                'order'      => 1,
            ],
            [
                'link'       => gettext('Upgrade'),
                'route_name' => 'admin.upgrade',
                'group'      => 5,
                'order'      => 2,
            ],
            [
                'link'       => gettext('Configuration'),
                'route_name' => 'admin.config',
                'group'      => 5,
                'order'      => 3,
            ],
            [
                'link'       => gettext('Members'),
                'route_name' => 'admin.members',
                'group'      => 5,
                'order'      => 3,
            ],
            [
                'link'       => gettext('Photo Gallery'),
                'route_name' => 'admin.photos',
                'group'      => 5,
                'order'      => 4,
            ],
            [
                'link'       => gettext('Polls'),
                'route_name' => 'admin.polls',
                'group'      => 5,
                'order'      => 5,
            ],
            [
                'link'       => gettext('Facebook'),
                'route_name' => 'admin.facebook',
                'group'      => 5,
                'order'      => 6,
            ],
            [
                'link'       => gettext('Google'),
                'route_name' => 'admin.google',
                'group'      => 5,
                'order'      => 7,
            ],
            [
                'link'       => gettext('Instagram'),
                'route_name' => 'admin.instagram',
                'group'      => 5,
                'order'      => 8,
            ],
        ];

        $communicatePlugins = [
            'familynews' => [
                'link'       => gettext('Family News'),
                'route_name' => 'familynews',
                'group'      => 2,
            ],
            'prayers' => [
                'link'       => gettext('Prayer Concerns'),
                'route_name' => 'prayers',
                'group'      => 2,
            ],
        ];
        $sharePlugins = [
            'recipes' => [
                'link'       => gettext('Recipes'),
                'route_name' => 'recipes',
                'group'      => 3,
            ],
            'familytree' => [
                'link'       => gettext('Family Tree'),
                'route_name' => 'familytree',
                'group'      => 3,
            ],
            'documents' => [
                'link'       => gettext('Documents'),
                'route_name' => 'documents',
                'group'      => 3,
            ],
        ];

        $communicateOrder = 3;
        $shareOrder       = 4;

        if ($request->has('sections'))
        {
            foreach ($request->sections as $key => $section)
            {
                if (isset($communicatePlugins[$section]))
                {
                    $insertParams[] = [
                        'link'       => $communicatePlugins[$section]['link'],
                        'route_name' => $communicatePlugins[$section]['route_name'],
                        'group'      => $communicatePlugins[$section]['group'],
                        'order'      => $communicateOrder,
                    ];
                    $communicateOrder++;
                }
                else if (isset($sharePlugins[$section]))
                {
                    $insertParams[] = [
                        'link'       => $sharePlugins[$section]['link'],
                        'route_name' => $sharePlugins[$section]['route_name'],
                        'group'      => $sharePlugins[$section]['group'],
                        'order'      => $shareOrder,
                    ];
                    $shareOrder++;
                }
            }
        }

        NavigationLink::insert($insertParams);

        return redirect()->route('install.admin');
    }

    /**
     * Show the admin form
     *
     * @return Illuminate\View\View
     */
    public function adminCreate()
    {
        $percent = round((3 / $this->totalSteps) * 100);

        $days   = [];
        $months = [];
        $years  = [];

        $d = 1;
        while ($d <= 31)
        {
            $days[$d] = $d;
            $d++;
        }
        $m = 1;
        while ($m <= 12)
        {
            $months[$m] = date('F', mktime(0, 0, 0, $m));
            $m++;
        }
        $y = 1900;
        while ($y + 13 <= gmdate('Y'))
        {
            $years[$y] = $y;
            $y++;
        }

        return view('install.admin', [
            'progress' => $percent,
            'days'     => $days,
            'months'   => $months,
            'years'    => array_reverse($years, true),
        ]);
    }

    /**
     * Store the admin user in the db
     *
     * @param  Illuminate\Http\Request $request
     * @return Illuminate\Support\Facades\View
     */
    public function adminStore(Request $request)
    {
        $validated = $request->validate([
            'email'    => ['required', 'email'],
            'password' => ['required', 'confirmed'],
            'name'     => ['required'],
            'bday'     => ['required', 'date'],
        ]);

        // Create the admin user
        $admin = new User;

        $admin->name      = $request->name;
        $admin->email     = $request->email;
        $admin->password  = Hash::make($request->password);
        $admin->birthday  = $request->bday;
        $admin->activated = true;
        $admin->access    = 1;

        if ($request->has('displayname'))
        {
            $admin->displayname = $request->displayname;
        }

        $admin->save();

        // Log the admin user in
        Auth::login($admin);

        // Create the user settings for the admin user
        $settings = new UserSetting;

        $settings->user_id = $admin->id;

        $settings->save();

        // Create an address for the admin user
        $address = new Address;

        $address->user_id         = $admin->id;
        $address->created_user_id = $admin->id;
        $address->updated_user_id = $admin->id;

        $address->save();

        return redirect()->route('home');
    }
}
