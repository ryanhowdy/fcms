...
if (isset($_POST['enter'])) {
    $sql = "DELETE FROM fcms_chat_users WHERE user_name = '" . cleanInput($_POST['name']) .  "'";
    mysql_query($sql) or die($sql.'<br/>'.mysql_error());
    $sql = "INSERT INTO fcms_chat_users(user_name,time) VALUES ('" . cleanInput($_POST['name']) .  "', NOW())";
    mysql_query($sql) or die($sql.'<br/>'.mysql_error());
}
// User left the chat
if (isset($_POST['exit'])) {
...
if (isset($_GET['chat'])) {
    ...
    $name = isset($_GET['name']) ? cleanInput($_GET['name']) : cleanInput($_POST['name']);
    ...
    if ($first) {
        $sql = "SELECT * FROM (
                    ...
                    AND message_id > " . cleanInput($last) . "
