...
class Admin {
	...
	function displayAdminConfig () {
		...
		echo "\t\t\t\t<div class=\"field-row clearfix\"><div class=\"field-label\"><label for=\"contact\">".$LANG['contact'].":</label></div><div class=\"field-widget\"><input type=\"text\" id=\"contact\" name=\"contact\" size=\"50\" value=\"" . $row['contact'] . "\"/></div></div>\n";
		...
		$recipes_exists = false; $news_exists = false; $prayers_exists = false; $documents_exists = false;
		...
		echo "\t\t\t\t<div class=\"cfg-sections clearfix\"><span class=\"newnews\">".$LANG['link_familynews']."</span>";
		if (!$news_exists) { echo "<a class=\"add\" href=\"?addsection=news\">".$LANG['add']."</a>"; } else { echo "<small>".$LANG['already_added']."</small>"; }
		echo "</div>\n\t\t\t\t<div class=\"cfg-sections clearfix\"><span class=\"newprayer\">".$LANG['link_prayers']."</span>";
		...
		echo "</div>\n\t\t\t\t<div class=\"cfg-sections clearfix\"><span class=\"newdocument\">".$LANG['link_documents']."</span>";
		if (!$documents_exists) { echo "<a class=\"add\" href=\"?addsection=documents\">".$LANG['add']."</a>"; } else { echo "<small>".$LANG['already_added']."</small>"; }
	...
	function displaySectionDropdown ($which_nav, $which_selected) { 
		global $LANG; ?>
				<div class="field-row clearfix">
					<div class="field-label"><label for="<?php echo $which_nav; ?>"><?php echo $LANG[$which_nav]; ?>:</label></div>
					<div class="field-widget">
						<select name="<?php echo $which_nav; ?>">
							<?php 
							if (tableExists('fcms_news')) { echo '<option value="familynews"'; if ($which_selected == 'familynews') { echo ' selected="selected"'; } echo '>' . $LANG['link_familynews'] . '</option>'; }
							if (tableExists('fcms_recipes')) { echo '<option value="recipes"'; if ($which_selected == 'recipes') { echo ' selected="selected"'; } echo '>' . $LANG['link_recipes'] . '</option>'; }
							if (tableExists('fcms_documents')) { echo '<option value="documents"'; if ($which_selected == 'documents') { echo ' selected="selected"'; } echo '>' . $LANG['link_documents'] . '</option>'; }
							if (tableExists('fcms_prayers')) { echo '<option value="prayers"'; if ($which_selected == 'prayers') { echo ' selected="selected"'; } echo '>' . $LANG['link_prayers'] . '</option>'; }
							$i = substr($which_nav, 7);
							echo '<option value="none'.$i.'"';
							$pos = strrpos($which_selected, "none");
							if ($pos !== false) { echo ' selected="selected"'; }
