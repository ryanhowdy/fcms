...
class Poll {
	...
	function displayResults ($pollid) {
		...
		while ($row = $this->db->get_row()) {
			...
			echo $row['option'] . "<span title=\"$votes ($percent%)\"class=\"poll\" style=\"width:$width" . "px" . ";\"></span>";
			$i++;
		}
		echo "</p><p>".$LANG['total_votes'].": $total</p><p><a href=\"?action=pastpolls\">".$LANG['past_polls']."</a></p>\n\t\t";
	}
