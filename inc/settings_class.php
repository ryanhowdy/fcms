...
class Settings {
	...
	function displayForm () {
		...
		if (is_dir($dir))	{
			if ($dh = opendir($dir)) {
				while (($file = readdir($dh)) !== false) {
					if (filetype($dir . $file) === "dir" && $file !== "." && $file !== ".." && $file !== "smileys") {
						$arr[] = $file;
					}
				}
				closedir($dh);
		...
		echo "<div class=\"field-row clearfix\"><div class=\"field-label\"><label for=\"sday\"><b>".$LANG['birthday']."</b></label>:</div><div class=\"field-widget\"><select id=\"sday\" name=\"sday\">";
		$d = 1;
		while($d <= 31) {
		...
		echo '</select><select id="smonth" name="smonth">';
		$m = 1;
		while($m <= 12) {
		...
		echo '</select><select id="syear" name="syear">';
		$y = 1900;
		while($y - 5 <= date('Y')) {
