...
class PhotoEditor
{
     ...
     * load 
     * 
     * Take a $_FILES object and sets the photo, filename and extension 
    ...
    public function save ($savedFileName = null)
    {
        ...
        $this->destinationType->copy($this->photo['tmp_name'], $this->fileName);
    ...
    public function resize ($maxWidth, $maxHeight, $resizeType = 'default')
    {
        ...
        if (!file_exists($this->destinationType->destinationPath.$this->fileName))
        {
            return $this;
        }
        ...
        if (!$this->destinationType->writeImage($destinationIdentifier, $this->fileName, $this->extension))
        {
            $this->fcmsError->add(array(
                'message' => T_('Upload Error'),
    ...
    public function rotate ($degrees = 90)
    {
        ...
        if (!$this->destinationType->writeImage($source, $this->fileName, $this->extension))
        {
            $this->fcmsError->add(array(
                'message' => T_('Upload Error'),
