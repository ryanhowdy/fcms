...
<div class="p-5">
    ...
    <div class="header d-flex">
        <img class="rounded-5 me-3" src="{{ getIndividualPicture($individual) }}" title="{{ _gettext('avatar') }}">
        <div>
            <h3 class="mb-0">
            ...
            @if ($individual['living'])
                <span class="text-indigo">{{ _gettext('Living') }}</span>
            @else
                {{ _gettext('Deceased') }}
            @endif
            </div>
            <a href="{{ route('familytree.showTree', $individual['id']) }}" class="btn btn-sm btn-light rounded-5">
                <i class="bi-diagram-3-fill pe-1"></i>{{ _gettext('View Tree') }}
            </a>
        </div>
    </div><!-- /.header -->

    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ _gettext('Name') }}</b></div>
        ...
        @if (!empty($individual['alias']))
            <div><b class="pe-1">{{ _gettext('Alias') }}:</b><i>{{ $individual['alias'] }}</i></div>
        @endif
        @if (!empty($individual['nickname']))
            <div><b class="pe-1">{{ _gettext('Nickname') }}:</b><i>{{ $individual['nickname'] }}</i></div>
        @endif
            <p>
                <a class="small" data-bs-toggle="collapse" href="#name-more" role="button" aria-expanded="false" aria-controls="name-more">
                    <i class="bi-chevron-right pe-1"></i>{{ _gettext('Events, Media, More') }}
                </a>
            </p>
            <div class="collapse small" id="name-more">
                <p>
                    <a href="{{ route('familytree.edit', $individual['id']) }}"><i class="bi-pencil pe-1"></i>{{ _gettext('Edit') }}</a>
                </p>
                <p>
                    <a href="{{ route('familytree.createEvent', $individual['id']) }}"><i class="bi-plus pe-1"></i>{{ _gettext('Add Event') }}</a>
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ _gettext('Birth') }}</b></div>
        <div class="content px-5 py-4">
            <div>
    ...
    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ _gettext('Death') }}</b></div>
        <div class="content px-5 py-4">
            <div>
            @if (!$individual['living'])
    ...
    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ _gettext('Description') }}</b></div>
        <div class="content px-5 py-4">
            <div>
