...
<div class="p-5">
    ...
    <div class="header d-flex">
        <img class="rounded-5 me-3" src="{{ getIndividualPicture($individual) }}" title="{{ __('avatar') }}">
        <div>
            <h3 class="mb-0">
            ...
            @if ($individual['living'])
                <span class="text-indigo">{{ __('Living') }}</span>
            @else
                {{ __('Deceased') }}
            @endif
            </div>
            <a href="{{ route('familytree.showTree', $individual['id']) }}" class="btn btn-sm btn-light rounded-5">
                <i class="bi-diagram-3-fill pe-1"></i>{{ __('View Tree') }}
            </a>
        </div>
    </div><!-- /.header -->

    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ __('Name') }}</b></div>
        ...
        @if (!empty($individual['alias']))
            <div><b class="pe-1">{{ __('Alias') }}:</b><i>{{ $individual['alias'] }}</i></div>
        @endif
        @if (!empty($individual['nickname']))
            <div><b class="pe-1">{{ __('Nickname') }}:</b><i>{{ $individual['nickname'] }}</i></div>
        @endif
            <p>
            ...
            <div class="collapse small" id="name-more">
                <p>
                    <a href="{{ route('familytree.edit', $individual['id']) }}"><i class="bi-pencil pe-1"></i>{{ __('Edit') }}</a>
                </p>
                <p>
                    <a href="{{ route('familytree.createEvent', $individual['id']) }}"><i class="bi-plus pe-1"></i>{{ __('Add Event') }}</a>
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ __('Birth') }}</b></div>
        <div class="content px-5 py-4">
            <div>
    ...
    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ __('Death') }}</b></div>
        <div class="content px-5 py-4">
            <div>
            @if (!$individual['living'])
    ...
    <div class="border-bottom">
        <div class="label py-4 text-end float-start"><b>{{ __('Description') }}</b></div>
        <div class="content px-5 py-4">
            <div>
