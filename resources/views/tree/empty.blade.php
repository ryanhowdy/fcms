...
<div class="p-5">
    ...
    <form class="p-5 border rounded bg-white" action="{{ route('familytree.create') }}" method="post">
        ...
        <h2 class="mb-3">{{ gettext('Start Family Tree') }}</h2>

        <div class="alert alert-info">
            <h3>{{ gettext('You do not have a Family Tree record yet.') }}</h3>
            <p>{{ gettext('Please verify the information below and make any changes or additions as needed.') }}</p>
            <p>{{ gettext('Notice that your Member record and Family Tree record are different.') }}</p>
        </div>
